...
bool            ParseMapEntity()
{
	...
	if (fabs (mapent->origin[0]) > g_iMaxEntityRange + ON_EPSILON ||
		fabs (mapent->origin[1]) > g_iMaxEntityRange + ON_EPSILON ||
		fabs (mapent->origin[2]) > g_iMaxEntityRange + ON_EPSILON )
	{
		...
		if (strncmp (classname, "light", 5))
		{
			Warning ("Entity %i (classname \"%s\"): origin outside +/-%.0f: (%.0f,%.0f,%.0f)", 
				g_numparsedentities, 
				classname, (double)g_iMaxEntityRange, mapent->origin[0], mapent->origin[1], mapent->origin[2]);
		}
	}
    return true;
}
